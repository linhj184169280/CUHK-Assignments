...
//Calculate the Euclidean Distance between 2 features
double Euclidean(Feature &fea1, Feature &fea2)
{
	if (fea1.data.size() != fea2.data.size())
	{
		return 1e64;
	}
	double distance = 0.0;
	for (int i = 0; i < fea1.data.size(); i++)
	{
		distance += (fea1.data[i] - fea2.data[i])*(fea1.data[i] - fea2.data[i]);
	}
	return distance;
}
