...
static void SetImageAlpha(CImg<float>& img, float blendRadius)
{
	...
	int nBands = img.spectrum();
	if (nBands != 4)
	{
		cout << "NormalizeBlend: Not a 4 Channels image." << endl;
		return;
	}
	...
	{
		float wx = 1.0f - abs(x - cx) / (width / 2.0f);
		float wy = 1.0f - abs(y - cy) / (height / 2.0f);
...
static void AccumulateBlend(CImg<float>& img, CImg<float>& acc, float blendRadius)
{
	...
	for (int y = 0; y < height; y++)
		for (int x = 0; x < width; x++)
	{
		float alpha = img(x,y,0,3) / 255.f;
		for (int c = 0; c < 3; c++)
		{
			acc(x, y, 0, c) = acc(x, y, 0, c) + img(x, y, 0, c) / 255.f * alpha;
		}
		acc(x, y, 0, 3) = acc(x, y, 0, 3) + alpha;
	}
	// *** END TODO ***
}
...
static void NormalizeBlend(CImg<float>& acc, CImg<float>& img)
{
	...
	for (int y = 0; y < height; y++)
		for (int x = 0; x < width; x++)
		{
			for (int c = 0; c < 3; c++)
			{
				img(x, y, 0, c) = (uchar)((acc(x, y, 0, c) / acc(x, y, 0, 3)) * 255);
			}
			img(x, y, 0, 3) = 255;
		}
